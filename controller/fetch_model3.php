<?php
	...
	$sql = "SELECT * FROM product WHERE product_name = '".$_POST["product_name"]."' group by `part_number`";
	...
	$output .= '<option value="" disabled selected>-- Part Number --</option>';
	...
	if(mysqli_num_rows($res)>0){
		...
		while ($row = mysqli_fetch_array($res)) {
			...
			$output .= '<option value="'.$row["part_number"].'">'.$row["part_number"].'</option>';
		}
	}
