<?php
	...
	$sql = "SELECT * FROM pressing WHERE operation_name = '".$_POST["operation_name"]."' ";
	...
	$output .= '<option value="" disabled selected>-- Quantity of Sub Part --</option>';
	...
	if(mysqli_num_rows($res)>0){
		...
		while ($row = mysqli_fetch_array($res)) {
			...
			$output .= '<option value="'.$row["quantity"].'">'.$row["quantity"].'</option>';
		}
	}
