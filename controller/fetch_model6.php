<?php
	...
	$sql = "SELECT * FROM product WHERE product_name = '".$_POST["product_quantity"]."' ";
	...
	//Concatenate fetched items to the output variable with HTML option tags to display
	 $output .= '<option value="" disabled selected>--Available Quantity--</option>';
	...
	if(mysqli_num_rows($res)>0){
		...
		while ($row = mysqli_fetch_array($res)) {
			...
			$output .= '<option value="'.$row["product_quantity"].'">'.$row["product_quantity"].'</option>';
		}
	}
